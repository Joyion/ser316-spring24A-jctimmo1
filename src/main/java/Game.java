 ...
 * Every game starts with a score of 10 and
 * the points are reduced based on the description of "makeGuess".
...
public class Game {
  ...
  //  private String leaderboard = "leaderboard.txt";
    ...
    public int countCorrectLetters() {
        int result = 0;
        if (!guesses.isEmpty()) {
            // Is it supposed to count each letter even
            // if the same letter or increment on unique letter?
     ...
     *  If word is correct return 0.0 and add points based on the length of the word
     *      (e.g. dog - 3 points, horse - 5 points), set game status to won
     ...
     * For either:
     *  Guess was already used, reduce points by 2 and return 4.0 (checked before 4.1 error).
     *      Guess still counts toward made guesses.
     *  Guess includes numbers/symbols etc. (so more than just letters) reduce points by 3 and return 4.1,
     *     the guess is still added to the list of guesses
     * Score can also be negative, that is no problem.
     * When the player guessed 10 times and did not guess the word set the game to game over (status)
     *  and return 5.0 (no matter if there was another error).
